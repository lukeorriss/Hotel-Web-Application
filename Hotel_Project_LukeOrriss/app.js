...
app.post('/payment', async function(req,res){
	 ...
	 await client.query(qOne).then(results =>{      //runs the queries
		 ...
		 client.query(qTwo).then( results =>{ 
			for (let i =0; i< parseInt(std_d); i++){         //iterates through each std_d room and allocates room number 
			const qThree = `SET SEARCH_PATH to hotelbooking; 
			SELECT MIN(rb.r_no) as rchosen from roombooking as rb, room as r  
			WHERE rb.r_no NOT IN (SELECT r_no from roombooking WHERE checkin <= '${checkOut}' AND '${checkIn}'<= checkout) AND r.r_class = 'std_d';` 
			...
			}); // the next for loops do the same as above for each room type 
			}
			for (let i =0; i< parseInt(std_t); i++){
				const qThree = `SET SEARCH_PATH to hotelbooking; SELECT MIN(rb.r_no) as rchosen from roombooking as rb, room as r  WHERE rb.r_no NOT IN (SELECT r_no from roombooking WHERE checkin <= '${checkOut}' AND '${checkIn}'<= checkout) AND r.r_class = 'std_t';` 
				client.query(qThree).then(results =>{
					data = results[1].rows;
			...
			for (let i =0; i< parseInt(sup_d); i++){
				const qThree = `SET SEARCH_PATH to hotelbooking; SELECT MIN(rb.r_no) as rchosen from roombooking as rb, room as r  WHERE rb.r_no NOT IN (SELECT r_no from roombooking WHERE checkin <= '${checkOut}' AND '${checkIn}'<= checkout) AND r.r_class = 'sup_d';` 
				client.query(qThree).then(results =>{
					data = results[1].rows;
					var roomNo= data[0].rchosen;
					console.log(roomNo)
			...
			for (let i =0; i< parseInt(sup_t); i++){
				const qThree = `SET SEARCH_PATH to hotelbooking; SELECT MIN(rb.r_no) as rchosen from roombooking as rb, room as r  WHERE rb.r_no NOT IN (SELECT r_no from roombooking WHERE checkin <= '${checkOut}' AND '${checkIn}'<= checkout) AND r.r_class = 'sup_t';` 
				client.query(qThree).then(results =>{
					data = results[1].rows;
					var roomNo= data[0].rchosen;
					console.log(roomNo)
